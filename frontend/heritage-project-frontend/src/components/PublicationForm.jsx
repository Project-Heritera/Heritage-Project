...
import { X, CheckCircle, Plus, Trash2, Shield, UploadCloud } from 'lucide-react';
...
function conditionalProps(props, propName, componentName) {
    ...
    // Helper to check integers
    const isInt = (val) => Number.isInteger(val) && val !== null;

    if (FormType === "Course" && !isInt(course_id)) {
        return new Error(`${componentName}: course_id must be an integer when FormType is 'Course'. Received ${course_id}`);
    }

    if (FormType === "Section" && (!isInt(course_id) || !isInt(section_id))) {
        return new Error(`${componentName}: both course_id and section_id must be integers when FormType is 'Section'.`);
    }

    if (FormType === "Room" && (!isInt(course_id) || !isInt(section_id) || !isInt(room_id))) {
        return new Error(`${componentName}: course_id, section_id, and room_id must be integers when FormType is 'Room'.`);
    }
    return null;
}

// --- MAIN COMPONENT: PublicationForm (Renamed to App for single-file structure) ---

function App({ FormType = "Room", course_id = 101, section_id = 202, room_id = 303 }) {
    ...
    // Set a mock user ID for the prompt text
    const resourceName = FormType.toLowerCase();

    const { fields, append, remove } = useFieldArray({
        control,
        name: "tags", // Changed from Access_To to tags based on form structure
    });
    ...
    useEffect(() => {
        // Simple logic to clear error on interaction, though not strictly tied to inputs.
        // A more complex approach would involve watching form state.
        if (errorMsg && !isLoading && !isPublished) {
            const timer = setTimeout(() => setErrorMsg(null), 5000);
            return () => clearTimeout(timer);
        }
    }, [errorMsg, isLoading, isPublished]);
    ...
    const onSubmit = async (data) => {
        ...
        try {
            ...
            let publish_status;
            if (FormType === "Room") {
                publish_status = await publish_room(course_id, section_id, room_id, publish_data);
            } else if (FormType === "Section") {
                publish_status = await publish_section(course_id, section_id, publish_data);
        ...
        <div className="flex items-center justify-center min-h-screen bg-gray-50 p-4 font-sans">
            <div className="w-full max-w-lg bg-white shadow-xl rounded-xl p-8 transition-all duration-300">
                ...
                <p className="text-gray-500 mb-6 border-b pb-4">
                    Define metadata for publishing this **{resourceName}** to the public catalog.
                </p>
                ...
                {isPublished && (
                    <div className="text-center p-8 bg-green-100 border-2 border-green-400 rounded-xl shadow-lg">
                        ...
                        <p className="text-gray-600 mb-6">
                            Your **{resourceName}** is now available in the public catalog.
                        </p>
                        <button
                            onClick={handleClose}
                ...
                {errorMsg && (
                    <div className="p-4 mb-4 bg-red-100 border border-red-400 text-red-700 rounded-lg transition-opacity duration-300">
                ...
                {!isPublished && !isLoading && (
                    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                        {/* Title */}
                        <div>
                            <label htmlFor="title" className="block text-sm font-medium text-gray-700">Title <span className="text-xs text-gray-400">(Max 100 chars)</span></label>
                            <input
                                id="title"
                                type="text"
                                placeholder={`Enter title for the ${resourceName}`}
                        ...
                        {/* Description */}
                        <div>
                            <label htmlFor="description" className="block text-sm font-medium text-gray-700">Description <span className="text-xs text-gray-400">(Max 225 chars)</span></label>
                            <textarea
                                id="description"
                                rows="3"
                                placeholder={`Provide a brief description of this ${resourceName}`}
                                className="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500 transition duration-150"
                                {...register("description", { required: "Description is required", maxLength: 225 })}
                            />
                        </div>
                        ...
                        {/* Badge Title */}
                        <div>
                            <label htmlFor="badge_title" className="block text-sm font-medium text-gray-700">Badge Title</label>
                            <input
                                id="badge_title"
                                type="text"
                                placeholder="e.g., Completion Badge"
                                className="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500 transition duration-150"
                                {...register("badge_title", { required: "Badge Title is required", maxLength: 225 })}
                            />
                        </div>

                        {/* Badge Icon */}
                        <div>
                            <label htmlFor="badge_icon" className="block text-sm font-medium text-gray-700">Badge Icon</label>
...
// Ensure PropTypes remain defined for the component
App.propTypes = {
    FormType: PropTypes.oneOf(["Course", "Section", "Room"]).isRequired,
    // The validator is assigned to all conditional IDs for redundancy as in the original code
